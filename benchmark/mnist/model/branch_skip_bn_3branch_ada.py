...
class Model(FModule):
    def __init__(self):
        ...
        self.branch2_layer23 = nn.Sequential(
            nn.Conv2d(20, 20, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(20),
            # nn.ReLU(),
        )
    ...
    def forward(self, x, n=0):
        ...
        #     x = self.branch2_bn2(x)
            ...
            x = x + x * self.branch2_layer22[1].weight.data.view(1,-1,1,1) +self.branch2_layer22[1].bias.data.view(1,-1,1,1)\
                + x * self.branch2_layer23[1].weight.data.view(1,-1,1,1) +self.branch2_layer23[1].bias.data.view(1,-1,1,1) 
